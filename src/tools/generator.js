...
class PasswordGenerator {
    ...
    canBeChanged(char) {
        return !regExps[types.UPPERCASE].test(char) && !regExps[types.NUMBER].test(char) && !regExps[types.SYMBOL].test(char);
    }
    ...
    substituteChar(string, index, type) {
        ...
        case types.NUMBER:
            if(this.canBeChanged(newString[index])) {
                newString = newString.substr(0, index) + this.generateRandomInt() + newString.substr(index + 1);
            }
            return newString;
        case types.SYMBOL:
            if(this.canBeChanged(newString[index])) {
                newString = newString.substr(0, index) + this.generateRandomSymbol() + newString.substr(index + 1);
            }
            return newString;
        case types.UPPERCASE:
            if(this.canBeChanged(newString[index])) {
                newString = newString.substr(0, index) + newString[index].toUpperCase() + newString.substr(index + 1);
            }
            return newString;
        default:
            return string;
    ...
    generateSecurePassword() {
        ...
        if (this.useUpperCase) {
            ...
            for (let i = 0; i < howManyUpperCase; i++) {
                password = this.substituteChar(password, this.getRandom(0, password.length), types.UPPERCASE);
            }
        }
        if (this.useNumber) {
            const howManyNumbers = this.getAmountOfSubstitutes();
            for (let i = 0; i < howManyNumbers; i++) {
                password = this.substituteChar(password, this.getRandom(0, password.length), types.NUMBER);
            }
        }
        if (this.useSymbol) {
            const howManySymbols = this.getAmountOfSubstitutes();
            for (let i = 0; i < howManySymbols; i++) {
                password = this.substituteChar(password, this.getRandom(0, password.length), types.SYMBOL);
            }
        }
        return password;
    }
